...
describe('Middleware', () => {
  ...
  describe(`multer should`, () => {
    /*  ----- This is not testable, Multer return function and I can't mock multer because it's const
    const multerMock = {
      multer: multer
    };
    const multerConfig = {
    ...
    it(`call multer with multerconfig properties uploadDirectory => dest and limits => limits and Middleware.controlMimeType => fileFilter`, () => {
      spyOn(multerMock, 'multer');
      console.log('TEST')
      Middleware.multer(req, res, methodProp.next, multerConfig);
      console.log('TESTF')
      expect(multerMock.multer).toHaveBeenCalledWith({
