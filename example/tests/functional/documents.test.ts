...
describe.each(databases)('%s - Documents - Error Cases', (_dbName, baseUrl) => {
  let token: string;
  ...
  it('POST /documents - should return 401 without token', async () => {
    const formData = new FormData();
    formData.append('name', 'test-document');
    formData.append('description', 'A test document');
    formData.append('fileDocument', new Blob([TINY_PNG], { type: 'image/png' }), 'test.png');
  ...
  it('POST /documents - should return 500 for invalid MIME type', async () => {
    const formData = new FormData();
    formData.append('name', 'test-document');
    formData.append('description', 'A test document');
    formData.append('fileDocument', new Blob(['fake content'], { type: 'image/jpeg' }), 'test.jpg');
